...
(() => {
    ...
    const sample = '[예시] 학생 A: AI 답변의 출처가 보이지 않으면 신뢰하기 어렵다고 말했다.\n이 내용은 입력을 분석한 결과가 아닌 준비된 가상 예시입니다.';
    ...
    window.fetch = async (input, options = {}) => {
        ...
        if (path === '/api/history') return json({success: true, data: [
            {title: '[예시] 멘토 업무 인사이트 예시', type: 'Example', status: 'Success', date: '2026-01-01T09:00:00'}
        ]});
        ...
        if (path === '/api/run/direct/analyze') return json({success: true, students: [{name: '[예시] 학생 A', content: sample}]});
        ...
        if (/^\/api\/run\/(auto|batch|summarize|direct)$/.test(path)) {
            const logs = [
                '[예시] 준비된 예시를 표시합니다. 입력 분석·메일 조회·Notion 기록은 하지 않습니다.',
                sample,
                ...
                '[예시] 처리 완료 — 실제 데이터는 생성하거나 전송하지 않았습니다.'
            ];
