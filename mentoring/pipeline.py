...
import os
import sys
from mentoring.config import INBOX_DIR, ARCHIVE_DIR, course_database_id, review_queue_id
...
from mentoring.services.ingestion_tracker import IngestionTracker
from mentoring.integrations.google_workspace import GoogleWorkspaceClient
from mentoring.services.batch_processor import BatchProcessor
...
async def run_pipeline(mode="auto", payload=None):
    ...
    global_course_db_id = course_database_id()
        ...
        try:
            ...
            for note in notes:
                await process_source(
                    ...
                    global_review_queue_id=review_queue_id()
                )
        except Exception as e:
            print(f"구글 연동 에러: {e}")
